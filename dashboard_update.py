...
.news-filter-btn.active{{background:#2a7af5;border-color:#2a7af5;color:#fff;font-weight:600}}
...
function buildNewsHtml(fundFilter,onlySummary){{
  /* Build per-company cards sorted by most recent article */
  var companies=[];
  ...
  /* Sort companies: newest article first */
...
var _newsShowOnlySummary=false;
function renderNews(){{
  var f=FUNDS_DATA[currentFundIdx];
  var filterBar=document.getElementById('news-filter-bar');
  if(filterBar)filterBar.remove();
  var bar=document.createElement('div');
  bar.id='news-filter-bar';
  bar.style.cssText='display:flex;gap:8px;padding:0 0 12px 0;';
  var btnAll=document.createElement('button');
  btnAll.textContent='Alle News';
  btnAll.className='news-filter-btn'+(!_newsShowOnlySummary?' active':'');
  btnAll.onclick=function(){{_newsShowOnlySummary=false;renderNews();}};
  var btnSum=document.createElement('button');
  btnSum.textContent='Nur mit Zusammenfassung';
  btnSum.className='news-filter-btn'+(_newsShowOnlySummary?' active':'');
  btnSum.onclick=function(){{_newsShowOnlySummary=true;renderNews();}};
  bar.appendChild(btnAll);bar.appendChild(btnSum);
  var container=document.getElementById('news-container');
  container.parentNode.insertBefore(bar,container);
  container.innerHTML=buildNewsHtml(f.id,_newsShowOnlySummary);
}}
